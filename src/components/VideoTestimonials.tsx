...
const VideoTestimonials = ({
  ...
  if (isLoading) {
    return <section className={`relative bg-foreground ${isFullPage ? "pt-16" : "py-16 md:py-24"}`}>
  ...
  return <section className={`relative bg-foreground ${isFullPage ? "pt-16" : "py-16 md:py-24"}`}>
      {!isFullPage && <div className="container mx-auto px-4 mb-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-background mb-4">
            Client <span className="text-primary">Stories</span>
          </h2>
          <p className="text-background/70 max-w-2xl mx-auto mb-4 px-0">Discover what we have delivered to our clients through impactful design transformations.</p>
          <Button variant="outline" className="border-primary text-primary hover:bg-primary hover:text-primary-foreground" onClick={() => navigate("/testimonials")}>
            View All Stories
          </Button>
        </div>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map(testimonial => <div key={testimonial.id} className="flex flex-col bg-background/5 rounded-xl overflow-hidden hover:bg-background/10 transition-colors">
              ...
              border: 0
                    ...
                    {/* Play Button Overlay */}
                    <button onClick={() => handlePlayClick(testimonial.id)} className="absolute inset-0 flex items-center justify-center bg-foreground/30 hover:bg-foreground/40 transition-colors group" aria-label={`Play ${testimonial.client_name}'s testimonial`}>
                    ...
                    <div className="absolute top-3 left-3 flex items-center gap-2 flex-wrap">
                      ...
                      {testimonial.location && <span className="inline-flex items-center gap-1 px-2 py-1 bg-background/20 backdrop-blur-sm text-background text-xs rounded-full">
              ...
              <div className="p-4 flex flex-col flex-grow">
                ...
                <StarRating rating={testimonial.star_rating || 5} size="sm" className="mb-2" />
                ...
                {/* CTA Buttons */}
                <TestimonialCTA variant="card" projectType={testimonial.project_type} clientName={testimonial.client_name} testimonialId={testimonial.id} pageSource={isFullPage ? "Testimonials Page" : "Home Page"} />
              </div>
            </div>)}
        </div>
      </div>
      ...
      {/* Trust Badges */}
      {showTrustBadges && <div className="mt-10">
